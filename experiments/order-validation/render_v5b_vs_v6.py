"""Render side-by-side overlays comparing v5b (RCM ablation) and v6
(v5b + 1 epoch fine-tune on the dln_focus pivot data).

Inputs:
    v5b ONNX + v6 ONNX
Outputs:
    /tmp/v5b_vs_v6/<label>.png — single PNG per page, v5b left, v6 right.

Same 8 diagnostic pages as render_v4_vs_v5b.py — the pages where
v4/v5b under-detected and motivated v6's data-distribution pivot.
...
def main() -> int:
    ...
    p.add_argument("--output", type=Path, default=Path("/tmp/v5b_vs_v6"))
    ...
    for label, src, page in test_pages:
        try:
            ...
            print(f"  {label}: v5b={len(v5b_ordered):>3}  v6={len(v6_ordered):>3}  "
                  f"(delta {len(v6_ordered) - len(v5b_ordered):+d})")
            out = side_by_side(image, v5b_ordered, v6_ordered,
                               label_a=f"v5b ({len(v5b_ordered)} dets)",
                               label_b=f"v6 ({len(v6_ordered)} dets)")
