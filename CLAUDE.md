...
**GPU acceleration (opt-in, Core 0.60.0+)**: `RailReader.Core.Analysis.WebGpu` wraps ONNX Runtime's native WebGPU execution provider (Dawn → D3D12/Vulkan/Metal, no vendor SDK; desktop-only, not folded into `Core.Analysis`). Only Docling Heron and PP-DocLayoutV3 have a GPU (FP16) model export (`LayoutModelRegistry.HeronFp16`/`PPDocLayoutV3Fp16`, ~9.5x/~7.3x measured speedup on a laptop GPU) — PP-DocLayout-S and the custom-model path always run on CPU. Persisted app-side as `CustomLayoutModelConfig.Accelerator` (`ConfigDir/custom_layout_model.json` — Core's `AppConfig` has no field for it, same reasoning as `BuiltinAnalyzer`), toggled in Settings ▸ **Advanced** ▸ "GPU Acceleration". `CustomLayoutModelLoader.TryResolveGpu` resolves the FP16 descriptor via `LayoutModelRegistry.Resolve(architecture, AcceleratorPreference.Gpu)` and probes `WebGpuAccelerator.IsAvailable`/the FP16 file on disk *before* committing to GPU, falling back to the CPU descriptor otherwise. Because `InitializeWorker`'s `analyzerFactory` runs lazily on the analysis worker's own thread (not at resolve time), the actual `WebGpuAccelerator.TryEnable`/`LayoutAnalyzerFactory.Create`/`Disable(architecture)` sequence — under `WebGpuAccelerator.ConstructionLock` — lives inside that deferred factory, with an inline CPU retry if GPU construction throws (a `LayoutModelRegistry`-resolved CPU model is a call away without holding the lock a second time). **No live hot-swap** — same "restart to apply" convention as the OCR language pack and layout-model picker, since the execution provider is fixed at `InferenceSession` construction. Download the FP16 models via `./scripts/download-model.sh heron-fp16`/`ppdoc-fp16` or the in-app "Download GPU model" button (writes to `ConfigDir/models`, same as the CPU models).

**Known issue**: the WebGPU EP (`Microsoft.ML.OnnxRuntime.EP.WebGpu`, still 0.x) can throw mid-session on a specific page's inference (observed: `GetElementType is not implemented` on an `Add` node, RTX A2000/Linux/Vulkan, Heron FP16) — `AnalysisWorker`'s per-request catch swallows it and keeps the worker alive for other pages, but that page silently gets zero detected blocks (missing paragraphs/blocks, not a crash). CPU-only analysis of the same page is unaffected — this is GPU/WebGPU-EP-specific. Tracked upstream at [RailReaderCore#108](https://github.com/sjvrensburg/RailReaderCore/issues/108); the Settings UI warns about it. No per-page CPU fallback exists yet (would need a Core change to `AnalysisWorker`'s catch path).
